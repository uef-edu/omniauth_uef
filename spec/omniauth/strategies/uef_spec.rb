...
RSpec.describe OmniAuth::Strategies::UefId do
  let(:body) {
    {
  ...
  context 'client options' do
    subject do
      ...
      OmniAuth::Strategies::UefId.new('uefid', 'Example-Client', 'b53c572b-9f3b-4e79-bf8b-f03c799ba6ec',
        client_options: {site: 'http://localhost:8080/', realm: 'example-realm'})
    end

    it 'should have the correct uef token url' do
      subject.setup_phase
      expect(subject.token_url).to eq('/auth/realms/example-realm/protocol/openid-connect/token')
    end

    it 'should have the correct uef authorization url' do
      subject.setup_phase
    ...
    context 'to invalid string' do
      ...
      it 'raises Configuration Error' do
        expect{ subject.setup_phase }
          .to raise_error(OmniAuth::Strategies::UefId::ConfigurationError)
      end
    end
  ...
  context 'client setup with a proc' do
    subject do
      OmniAuth::Strategies::UefId.new('uefid', setup: proc { throw :setup_proc_was_called })
    end
    ...
    context 'when raise_on_failure option is true' do
      context 'when openid configuration endpoint returns error response' do
        ...
        it 'raises Integration Error' do
          expect{ subject.setup_phase }
            .to raise_error(OmniAuth::Strategies::UefId::IntegrationError)
        end
      end
